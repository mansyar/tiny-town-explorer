...
3.  **Test-Driven Development (scoped):** Write unit tests before
    implementing **logic-bearing code only** — game rules, pathfinding,
    FSM, input resolution, mission pacing, audio scheduling. Visual,
    scene-setup, asset-loading, and UI-glue code are exempt from red/green
    discipline and verified through manual verification steps.
4.  **High Code Coverage (scoped):** Aim for >80% code coverage for
    logic-bearing modules; rendering, scene, and asset code are excluded
    from coverage targets.
...
3.  **Write Failing Tests (Red Phase) — logic-bearing tasks only:**
    ...
    -   **Scope check first:** If the task is visual, scene, asset, or
        UI-glue work (exempt per Guiding Principle 3), skip steps 3–4 and
        instead record the intended manual verification steps for the
        task summary.
...
-   [ ] All tests pass
-   [ ] Code coverage meets requirements (>80% for logic-bearing modules;
        visual/asset/scene code exempt)
...
# pnpm test -- --coverage   # Vitest with coverage (logic modules)
```

### Before Committing

```bash
# pnpm check && pnpm typecheck && CI=true pnpm test
```
...
-   Test on actual target tablets (iPad 9th-gen floor) when possible
...
3.  Code coverage meets project requirements (>80% logic-bearing modules)
